...
![](https://cdn.nlark.com/yuque/0/2021/png/2345482/1612022991019-7039f873-9104-4a3b-9f77-cef6b8697bd1.png#align=left&display=inline&height=65&originHeight=65&originWidth=644&size=0&status=done&style=stroke&width=644)
...
![](https://cdn.nlark.com/yuque/0/2021/jpeg/2345482/1612022991039-501baee8-a614-465f-9461-a9c4111fddc9.jpeg#align=left&display=inline&height=277&originHeight=277&originWidth=764&size=0&status=done&style=stroke&width=764)<br />从图中可以看出，当有序表的元素个数不是斐波那契数列中的某个数字时，需要把有序表的元素个数长度补齐，让它成为斐波那契数列中的一个数值，当然把原有序表截断肯定是不可能的，不然还怎么查找。然后图中标识每次取斐波那契数列中的某个值时(F[k])，都会进行-1操作，这是因为有序表数组位序从0开始的，纯粹是为了迎合位序从0开始。所以用迭代实现斐波那契查找算法如下：
#### 代码实现
```java
public static void main(String[] args) {
    int[] arr = {1, 8, 10, 89, 89, 1000, 1234};
