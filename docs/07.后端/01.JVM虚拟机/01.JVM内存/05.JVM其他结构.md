...
简单地讲，<span style="color:blue">一个Native Method就是一个Java调用非Java代码的接口</span>。一个Native Method是这样一个Java方兆:该方法的实现由非Java语言实现，比如C。这个特征并非Java所特有，很多其它的编程语言都有这一机制，比如在C++中，你可以用extern "C" 告知C++编译器去调用一个C的函数。
