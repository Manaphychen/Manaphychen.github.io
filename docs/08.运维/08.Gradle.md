...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1656928075850-667a7757-c867-42a6-88d3-bdecc3f1c3ce.png)
## 1.3 Gradle安装
...
## 1.4 Gradle 项目目录结构
Gradle 项目默认目录结构和 Maven 项目的目录结构一致,都是基于约定大于配置【Convention Over Configuration】。<br />其完整项目目录结构如下所示：<br />![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1656952651446-16be7956-c96c-47f6-be2b-89dd5db67bcf.png)
> 1. 只有war工程才有webapp目录，对于普通的jar工程并没有webapp目录
> 2. gradlew与gradlew.bat执行的指定wrapper版本中的gradle指令,不是本地安装的gradle指令哦。

## 1.5 Gradle 创建第一个项目
借助于 spring 脚手架创建 gradle 第一个项目：[https://start.spring.io/](https://start.spring.io/)<br />![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1656952772559-17478284-9ed9-40b2-a592-548967509000.png)<br />与上图对比会发现：总体的目录结构与上图说明的是一致的。
### 1.5.1 Gradle 中的常用指令
| 常用Gradle指令 | 作用 |
| :-- | :-- |
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1656954000880-ff4b143b-b998-4baf-b1cd-976339d11fea.png)<br />**gradle-wrapper.properties 文件解读**
...
### 默认测试目录及标准输出
![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657271987197-4c98f1f2-3fea-4620-9bcf-4d4d2bc45a1c.png)
### Junit 使用
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657288232862-71bda1f9-d73f-4c03-b5a5-e92a57018b92.png)
...
拓展：gradle 任务名是缩写: 任务名支持驼峰式命名风格的任务名缩写，如： connectTask 简写为： cT,执行任务 gradle cT。 <br />拓展 1：前面提到的 Gradle 指令本质：一个个的 task[任务]，Gradle 中所有操作都是基于任务完成的。 <br />拓展 2：gradle 默认各指令之间相互的依赖关系：<br />![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657352078289-06822ec3-c02d-4ce5-8933-d0aae9beb195.png)
### 3.3.5 任务定义方式
...
```
当然：我们也可以在定义任务的同时指定任务的属性，具体属性有：<br />![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657352287927-5f7a13e5-31da-411c-b2c6-6284e3c51766.png)<br />**在定义任务时也可以给任务分配属性**：定义任务的时候可以直接指定任务属性，也可以给已有的任务动态分配属性：
```groovy
...
```
![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657353196588-b9de41f7-080e-4fd8-b789-be4e933c2663.png)
### 3.3.6 任务类型
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657462531412-4e35b2cb-850f-42e1-a3a6-0dc0df038936.png)<br />总之，除非涉及到多模块依赖，为了避免重复依赖，咱们会使用 api,其它情况我们优先选择 implementation， 拥有大量 的 api 依赖项会显著增加构建时间。
### 3.5.5 依赖冲突及解决方案
依赖冲突是指 "在编译过程中, 如果存在某个依赖的多个版本, 构建系统应该选择哪个进行构建的问题",如下所示:<br />![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657462854629-2459cab6-5b8e-4507-8dff-7afef57da440.png)<br />A、B、C 都是本地子项目 module，log4j 是远程依赖。<br />编译时： B 用 1.4.2 版本的 log4j，C 用 2.2.4 版本的 log4j,B 和 C 之间没有冲突 打包时： 只能有一个版本的代码最终打包进最终的A对应的jar包，对于 Gradle 来说这里就有冲突了<br />案例演示：我们在 build.gradle 引入依赖库
```groovy
dependencies {
...
```
![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657463136580-fc3b5605-08f7-4116-95fb-970e9eb93215.png)<br />如上所示：默认下，Gradle 会使用最新版本的 jar 包【考虑到新版本的 jar 包一般都是向下兼容的】，实际开发中，还 是建议使用官方自带的这种解决方案。**当然除此之外，Gradle 也为我们提供了一系列的解决依赖冲突的方法**: exclude 移除一个依赖，不允许依赖传递，强制使用某个版本。
...
### 3.6.3 插件的分类和使用
![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657463772928-1f930fa3-7051-4c23-ad1a-8480196ff297.png)
#### 第一种：脚本插件
...
#### 第二种：对象插件之内部插件[核心插件]
二进制插件[对象插件]就是实现了 org.gradle.api.Plugin 接口的插件，每个 Java Gradle 插件都有一个 plugin id。<br />![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657463898076-e714abbe-06dc-4a28-8653-77b4cf811571.png)<br />可通过如下方式使用一个 Java 插件：<br />`apply plugin : 'java' //map具名参数方式`<br />或者：
```groovy
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657464614953-302f2666-c4b5-493e-bfc8-e1b4cc3ad95d.png)
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657464715942-dfbeaaba-e01b-4eec-a107-4ce8acba8a2d.png)
...
```
也可设置源集的属性等信息。<br />**第四点：依赖管理：以 java 插件为例，提供了很多依赖管理项**<br />![源集依赖关系配置](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657468318061-5c3e1b8b-644d-48d3-ba68-4f9f7087dc5f.png)

![测试源集依赖关系配置](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657468328442-fee1e233-c517-4762-b8d6-e452f156583e.png)
## 3.7 build.gradle 文件
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657468443035-ba9e155b-0a02-4256-8761-473da513260b.png)
### 3.7.1 常见属性代码
代码参考：
```groovy
...
```
## 3.8 publishing 项目发布
**接下来，将咱们写好的模块发布发布到公司的私服以供别人使用,如下所示：**<br />![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657469238964-8bc589b9-8e9c-4a12-b7fe-12916d2a42f8.png)
...
## 3.9 生命周期中 Hook
生命周期中的这些钩子函数都是由 gradle 自动回调完成的,利用这些钩子函数可以帮助我们实现一些我们想要的功能。<br />![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657469489339-f16b27f5-efc2-4797-ab0c-e5e44471144d.png)<br />Gradle 在生命周期各个阶段都提供了用于回调的钩子函数:<br />Gradle 初始化阶段:
...
钩子函数代码演示：项目目录结构如下：<br />![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657469572153-e49c1162-bcac-4651-9af1-96dd4c9a2bb1.png)<br />在 root project 的 settings.gradle 文件中添加：
```groovy
...
## 3.11 基于 ssm 多模块项目案例
### 3.11.1 多项目模块划分
![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657470316739-3b9b42d3-ccb9-412e-bdfa-5a10aa1b4d3e.png)<br />meinian-mobile-web: 美年旅游项目的用户系统<br />meinian-web:美年旅游项目的管理员系统<br />meinian-service:	美年旅游项目的业务逻辑层<br />meinian-dao:美年旅游项目的持久化层<br />meinian-bean:美年旅游项目的Model 封装
### 3.11.2 项目搭建前配置分析
![image.png](https://cdn.nlark.com/yuque/0/2022/png/2345482/1657470383156-3f408913-ab81-4320-aee2-a1a2136067f0.png)
### 3.11.3 代码演示
