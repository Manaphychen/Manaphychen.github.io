...
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209190904241.png)
...
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209191606156.png)
...
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209191902318.png)
...
### 1.6 Jenkins介绍
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208192124705.png)
...
### 2.1 持续集成流程说明
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201207202502055.png)
...
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208182525527.png)
...
#### 2.2.2 配置Gitlab帐号密码
访问http://192.168.2.136:8880 并设置好密码<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20200819174745049.png)
...
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208184013669.png)

2. 创建用户

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208184235480.png)

3. 将用户添加到组中

![](https://cdn.nlark.com/yuque/0/2021/png/2345482/1610517724469-0c883325-6fe9-48c9-b6ae-2ea51694e135.png)

4. 在用户组中创建项目<br />以刚才创建的新用户身份登录到Gitlab，然后在用户组中创建新的项目<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208185332371.png)
...
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208190032443.png)

3. 提交代码到本地仓库 `Git→提交`

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208190226122.png)

4. 推送到Gitlab项目仓库中 `Git→推送`

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208190557524.png)
...
### 2.5 全局工具配置
在`系统管理→全局工具配置`<br />**配置JDK**<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209100716036.png)

**配置Git**<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209100806739.png)

### 2.6 用户权限管理
> 利用Role-based Authorization Strategy插件来管理Jenkins用户权限


**安装 Role-based Authorization Strategy 插件**<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208193743195.png)

**开启权限全局安全配置**<br />进入`系统管理→全局安全配置` 授权策略切换为 `Role_Based Strategy` 保存<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208194244974.png)
#### 2.6.1 创建角色
在`系统管理→Manage and Assign Roles`<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208194404840.png)

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208194940956.png)
...
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208195410397.png)<br />保存
#### 2.6.2 创建用户
在`系统管理→管理用户`<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208195553286.png)
#### 2.6.3 给用户分配角色
...
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208200049990.png)<br />保存
#### 2.6.4 创建项目测试权限
待定

### 2.7 凭证管理
凭据可以用来存储需要密文保护的数据库密码、Gitlab密码信息、Docker私有仓库密码等，以便Jenkins可以和这些第三方的应用进行交互。<br />**安装Credentials Binding Plugin插件(已在推荐的插件列表中)**<br />在`系统管理→Manage Credentials`<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208201320469.png)
...
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208202242697.png)

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208202341036.png)

2. 测试凭证是否可用<br />构建一个自由风格的软件项目<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201208202445833.png)<br />查看`/root/docker/jenkins/data/workspace/test01/`目录<br />发现已经从Gitlab成功拉取了代码到Jenkins中
...
```
   ...
   - id_rsa: 私钥文件
   - id_rsa.pub: 公钥文件
2. 把生成的公钥放在Gitlab中<br />以root账户登录→点击头像→设置→SSH密钥<br />复制`id_rsa.pub`文件的内容到这里，点击添加密钥<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209115807931.png)
3. 在Jenkins中添加凭证，配置私钥<br />在Jenkins添加一个新的凭证，类型为`SSH Username with private key`，并复制私钥文件内容到此处<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209133501836.png)

      ![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209133730269.png)

4. 测试凭证是否可用<br />新建test02项目→源码管理→Git，这次使用Gitlab的SSH连接，并且选择SSH凭证<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209135553100.png)<br />同样尝试构建项目，如果代码可以正常拉取，代表凭证配置成功

### 2.8 Maven安装和配置一(自动安装)
在Jenkins集成服务器上，我们需要安装Maven来编译和打包项目。<br />在**插件管理**安装`Maven Integration`插件<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209141044560.png)

在`系统管理→全局工具配置`中配置Maven<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209184706225.png)

**测试Maven是否可用**<br />修改test02项目，在构建中选`调用顶层Maven目标`<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209185346039.png)

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209185303605.png)<br />执行构建后查看控制台信息就能看到Jenkins自动下载Maven，进行构建
...
```
#### 2.9.2 全局工具配置关联Maven
在`全局工具中配置`中新增Maven配置<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210151608982.png)

#### 2.9.3 添加Jenkins全局变量
在`系统管理→系统配置→全局属性`中，添加**M2_HOME**和**PATH+EXTRA**两个属性<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210153634996.png)		
#### 2.9.4 测试是否配置成功
新建`test-mvn`项目 在`配置→构建`的`执行Shell`中添加`mvn -v` 保存后点构建<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210153911180.png)
...
重启Tomcat 访问测试<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210161459791.png)

#### 2.10.2 Jenkins构建Maven项目

1. 需要`Maven Integration`插件，已在前文安装
2. 创建Maven项目<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209194942039.png)
3. 配置项目<br />拉取代码和前文自由风格项目一样，只是**构建**部分不同

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209195213926.png)

#### 2.10.3 将项目部署到远程Tomcat

1. 安装`Deploy to container`插件<br />Jenkins本身无法实现远程部署到Tomcat的功能，需要安装Deploy to container插件实现  (过程略...)
2. 添加Tomcat用户凭证<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210162641681.png)
3. 添加构建后操作

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210163014243.png)

4. 部署成功后访问项目<br />[http://192.168.2.137:8080/springboot_jenkins-1.0/](http://192.168.2.137:8080/springboot_jenkins-1.0/)<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210164406698.png)
5. 改动代码后的持续集成
...
**Declarative声明式-Pipeline**<br />创建名为`pipeline01`的流水线项目<br />流水线→选择Hello World模板<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209200434905.png)
...
```
点击构建，可以看到整个构建过程<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209201027022.png)
...
构建结果和声明式一样
#### 3.2.3 拉取代码
新建`pipeline_demo01`流水线项目<br />点击流水线语法<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201209202922139.png)<br />点击`生成流水线脚本` 就会出现以下内容
```
...
1. 在项目根目录建立Jenkinsfile文件，把内容复制到该文件中，然后上传到Gitlab<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210165907257.png)

2. 在项目中引用该文件

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210170252051.png)

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210170353903.png)

### 3.3 常用的构建触发器
Jenkins内置的构建触发器<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210171434461.png)
#### 3.3.1 触发远程构建
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210171647878.png)
#### 3.3.2 其他工程构建后触发

1. 创建`pre_job`流水线工程

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210172458481.png)

2. 配置需要触发的工程

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210172607131.png)
#### 3.3.3 定时构建
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210172837415.png)<br />定时字符串从左往右分别为︰分时日月周<br />一些定时表达式的例子
...
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210174512982.png)
#### 3.4.1 安装 Gitlab Hook 插件
需要安装`GitLab`和`Gitlab Hook`两个插件
#### 3.4.2 设置自动构建
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210175953053.png)<br />等会需要把生成的webhook URL配置到Gitlab中。<br />**Gitlab配置webhook**

1. 开启Webhook功能<br />使用root账户登录到后台，如下图所示勾选

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210183137971.png)

2. 在项目添加webhook<br />在项目设置的`Webhooks`中<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210183932199.png)<br />注意: 以下设置必须完成，否则会报错<br />在`系统管理→系统配置`中

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210185050626.png)

3. 测试<br />提交一次代码，就会发现自动构件代码

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210190020367.png)

#### 3.4.3 参数化构建
> 有时在项目构建的过程中，我们需要根据用户的输入动态传入一些参数，从而影响整个构建结果，这时我们可以使用参数化构建。

Jenkins支持非常丰富的参数类型<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210193403775.png)

接下来演示通过输入gitlab项目的分支名称来部署不同分支项目。<br />新建分支 v1 ，然后改动代码提交到GitLab上；此时Gitlab上就会有两个分支

**在 Jenkins 添加文本类型参数**<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210193824544.png)

改动`pipeline`流水线代码

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210194054997.png)

**点击Build with Parameters**

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210194208022.png)

输入分支名称，构建即可！构建完成后访问Tomcat查看结果

#### 3.4.4 配置邮箱服务器发送构建结果

**安装 Email Extension Template 插件**<br />**Jenkins设置邮箱相关参数**<br />在`系统管理`中配置<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210201829517.png)

设置邮件参数

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201211094603659.png)

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201211094717508.png)

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201211094923991.png)

设置邮件通知

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201210202500577.png)
...
```
**测试**<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201211095910536.png)
### 3.5 安装SonarQube
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201211102635044.png)
...
```
访问 [http://192.168.2.137:9000/](http://192.168.2.137:9000/)<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201211103138503.png)
#### 2. 安装汉化包
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201211103738266.png)<br />安装完后重新启动服务
#### 3. 生成token
点击头像→我的帐号→安全→生成令牌<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201211104854108.png)
#### 4. 测试使用
...
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201211105145467.png)
#### 5. 安装 SonarQube Scanner 插件
过程略。。。
#### 6. 添加 SonarQube 凭证
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201211110541667.png)
#### 7. 在系统配置中的 SonarQube servers 中配置
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201211110913912.png)
#### 8.  SonaQube关闭审查结果上传到SCM功能
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201211111229432.png)
...
### 4.1 持续集成流程说明
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201207202502055.png)<br />大致流程说明︰
...
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201207101929130.png)
...
**创建项目**<br />Harbor的项目分为公开和私有的:<br />公开项目︰所有用户都可以访问，通常存放公共的镜像，默认有一个library公开项目。<br />私有项目∶只有授权用户才可以访问，通常存放项目本身的镜像。<br />我们可以为微服务项目创建一个新的项目∶<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201207204034308.png)
...
```
再次推送镜像<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201211200206233.png)
#### 4.3.4 从Harbor下载镜像
...
```
**拉取Jenkinsfile文件**<br />创建新项目<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201214105352661.png)
### 4.6 提交到SonarQube代码审查

1. 在项目配置中的`参数化构建过程`中添加两个参数

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201214110256918.png)

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201214110315606.png)
...
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201214113402525.png)
...
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201214135442341.png)

![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201215202002811.png)

#### 4.9.3 修改Jenkinsfile构建脚本
生成远程调用模板代码<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201214171758347.png)

添加一个`port`参数<br />![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201214171935865.png)
...
### 4.10 部署前端静态web网站
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201215203312807.png)
...
![](https://manaphy.oss-cn-hangzhou.aliyuncs.com/blogImg/image-20201215204042123.png)
