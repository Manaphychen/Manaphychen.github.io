    ...
    static_configs:
      - targets: ['192.168.0.200:9100'] #填写宿主机ip:映射的node-exporter端口
        labels:
          instance: localhost
```
使用`curl -XPOST localhost:9090/-/reload`刷新Prometheus<br />访问 [http://192.168.0.200:9090/targets](http://192.168.0.200:9090/targets) 查看是否有相应端点<br />![image.png](https://cdn.nlark.com/yuque/0/2023/png/2345482/1676540799667-e92647a2-e9e8-4e60-9668-963bc7c662a2.png)
### MysqldExporter
...
```
使用`curl -XPOST localhost:9090/-/reload`刷新Prometheus<br />访问 [http://192.168.0.200:9090/targets](http://192.168.0.200:9090/targets) 查看是否有相应端点<br />![image.png](https://cdn.nlark.com/yuque/0/2023/png/2345482/1676541569291-800c0909-1728-4c05-8e43-40f19d9f8fb6.png)
### RedisExporter
**安装部署**
```yaml
version: "3.1"
...
```
打开插件后可以在RabbitMQ默认的Web UI上可以看到暴露数据的端口默认是15692<br />![image.png](https://cdn.nlark.com/yuque/0/2023/png/2345482/1684331925843-685e3ef4-17a8-4a2f-a441-778c25b6f255.png)访问插件监听收集的指标数据：
```yaml
  # 监控 rabbitmq
  - job_name: 'rabbitmq'
    static_configs:
...
services:
  cadvisor:
    ...
    command: detach=true
```
访问 [http://192.168.0.200:8080/containers](http://192.168.0.200:8080/containers ) 可以看到相关信息<br />![image.png](https://cdn.nlark.com/yuque/0/2023/png/2345482/1676541915923-91a39d6e-f473-47e0-85e6-c7d765adf5b8.png)<br />在`prometheus.yml`中的`scrape_configs:`下添加以下内容
```yaml
  # 对接cadvisor
  - job_name: "docker"
    static_configs:
...
```
使用`curl -XPOST localhost:9090/-/reload`刷新Prometheus<br />访问 [http://192.168.0.200:9090/targets](http://192.168.0.200:9090/targets) 查看是否有相应端点<br />![image.png](https://cdn.nlark.com/yuque/0/2023/png/2345482/1676541999175-d162fd96-143c-407a-8330-c9df58fecdf2.png)
## Grafana
...
### 添加prometheus数据源
在`Configuration`->`Data sources`-> `Add data source` -> `Prometheus`<br />![image.png](https://cdn.nlark.com/yuque/0/2023/png/2345482/1676542866130-7fe455db-5903-4850-924b-7671a6623736.png)<br />点击 `Save & test`保存
### 导入控制台模板
官网模版地址：[https://grafana.com/grafana/dashboards](https://grafana.com/grafana/dashboards)<br />搜索相关模板添加 如: docker，选择一个模板，复制模板的地址<br />![image.png](https://cdn.nlark.com/yuque/0/2023/png/2345482/1676543024987-c854f6a5-4e2c-49d2-8619-dc508dbaa4a5.png)<br />点击保存
...
```
在Grafana中添加JVM相关的dashboards<br />![image.png](https://cdn.nlark.com/yuque/0/2023/png/2345482/1676543830084-5ad62625-89fa-4cf3-b207-4919f3f111bc.png)
## 监控Nacos
