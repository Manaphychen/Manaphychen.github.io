...
- redo log: 是存储引擎层(innodb)生成的日志， 记录的是"`物理级别`"上的页修改操作，比如页号xxx、 偏移量yy写入了'zz'数据。主要为了保证数据的可靠性;
- undo log: 是存储引擎层(innodb)生成的日志， 记录的是`逻辑操作`日志，比如对某一 -行数据进行了INSERT语句操作，那么undo log就记录-条与之相反的DELETE操作。主要用于`事务的回滚`(undo log记录的是每个修改操作的`逆操作`)和`一致性非锁定读`(undo log回滚行记录到某种特定的版本---MVCC,即多版本并发控制)。
## 1. redo 日志
...
![image-20220328151424082.png](./assets/ZuaUdt.png)
...
![image.png](./assets/JhYOQY.png)

除了1s 强制刷盘，page cache 由系统决定啥时候刷盘

::: tip 小结 innodb_ flush_ log_ at_trx_commit=2
...
![image.png](./assets/FTrOv1.png)

::: tip 小结: innodb_ flush_ log_at_trx_commit=0
...
![image.png](./assets/a8bf4f50c7e34458abce899e920540f6png.png)
...
![image-20220328142935235.png](./assets/ixacFQ.png)
...
![v2-ccc4f0ad502b8604d25ff00213853b54_1440w.jpg](./assets/eXiHPB.jpeg)
#### 2. 回滚段与事务
...
![image.png](./assets/D1INh3.png)

**有了Redo Log和Undo Log之后：**

![image.png](./assets/e2sOaS.png)
