...
<span class="line"><span>    .setValue(&quot;carbohydrate&quot;,27).build();</span></span></code></pre></div><p>为了简洁，省略了有效性检查。为了尽快检测无效的参数，请检查构建器的构造函数和方法中的参数有效性。检查构建方法调用的构造函数中涉及多个参数的不变量。为了确保这些不变量不受攻击，在从构建器复制参数之后检查对象字段（<a href="./50-Make-defensive-copies-when-needed">Item-50</a>）。如果检查失败，抛出一个 IllegalArgumentException（<a href="./72-Favor-the-use-of-standard-exceptions">Item-72</a>），它的详细消息指示哪些参数无效（<a href="./75-Include-failure-capture-information-in-detail-messages">Item-75</a>）。</p><p>The Builder pattern is well suited to class hierarchies. Use a parallel hierarchy of builders, each nested in the corresponding class. Abstract classes have abstract builders; concrete classes have concrete builders. For example,consider an abstract class at the root of a hierarchy representing various kinds of pizza:</p><p>建造者模式非常适合于类层次结构。使用构建器的并行层次结构，每个构建器都嵌套在相应的类中。抽象类有抽象类构建器；具体类有具体类构建器。例如，考虑一个在层次结构处于最低端的抽象类，它代表各种比萨饼：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark-dimmed vp-code" tabindex="0"><code><span class="line"><span>import java.util.EnumSet;</span></span>
